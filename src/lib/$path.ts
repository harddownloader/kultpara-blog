  ...
  _locale: (locale: string | number) => ({
    "post": {
      _slug: (slug: string | number) => ({
        $url: (url?: { hash?: string }) => ({ pathname: '/[locale]/post/[slug]' as const, query: { locale, slug }, hash: url?.hash })
      })
    },
    // _sitemap: (sitemap: string | number) => ({
    //   $url: (url?: { hash?: string }) => ({ pathname: '/[locale]/[sitemap]' as const, query: { locale, sitemap }, hash: url?.hash })
    // }),
    // "account": {
    //   "addressBook": {
